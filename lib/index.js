...
phantomjs.run('--webdriver=4444').then((() => {
    var _ref = _asyncToGenerator(function* (program) {
        ...
        let browser = yield webdriverio.remote(wdOpts).init();

        console.log('Webdriver loaded');

        yield browser.url('https://galaxy.meteor.com/app/new.skoolerstutoring.com');
        ...
        yield browser.pause(LOADING_TIMEOUT);
        ...
        yield browser.getText("div.cardinal-name=Connections").then(function (text) {
            console.log(text);
        });
